...
@section('head-tag')
<title>محصولات {{ cache('templateSetting')['title'] }}</title>
@endsection

@section('content')
<section class="container content lg:blur-0 my-20">
    <div class="flex items-center justify-between w-full my-7">
        <h1 class="text-lg font-bold lg:text-3xl text-secondary dark:text-white/80">همه دوره ها</h1>
...
{{--        </a>--}}
    ...
    <div class="flex flex-wrap w-full   gap-y-14 lg:gap-x-4 lg:w-4/5 h-min md:justify-between lg:justify-start" data-aos="fade-up">

        @forelse ( $courses as $course)
        <div
            class="relative top-10 flex flex-col items-center justify-center w-full  shadow-lg rounded-3xl dark:bg-dark   dark:shoadow-none lg:w-[32%] md:w-[49%] ">
            ...
            <div class="flex flex-col justify-start w-full gap-3 px-10 -mt-5 lg:px-3">
                <a href="{{ route('customer.course.singleCourse',$course->slug)}}">
                    <h3
                        ...
                        class="pb-2 text-lg lg:text-xl font-bold border-b text-secondary dark:text-white/80 border-b-main/30 hover:text-main hover:font-bold  hover:text-main hover:font-bold dark:hover:text-main">
                        {{ $course->title }}</h3>
                </a>
                </h3>
                ...
                <!--</div>-->
                    ...
                    <a class="px-4 py-2 text-sm font-bold text-white duration-200 bg-main/80 rounded-xl hover:bg-main hover:scale-95 flex items-center gap-2"
                                ...
                                ثبت نام در دوره
                                <svg  class="w-5 h-5"    stroke="currentColor" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M12 8V12M12 16V12M12 12H16H8" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
...
<g id="OBJECTS">
	...
	<g>
		...
		<path style="fill:#FFFFFF;" d="M302.604,333.539c1.292-3.725,4.122-6.738,7.526-8.83c3.426-2.077,7.494-3.192,11.563-3.191    c4.069-0.01,8.14,1.102,11.567,3.181c3.406,2.09,6.236,5.108,7.521,8.84c-3.722-1.257-6.869-2.252-9.996-2.86    c-3.101-0.629-6.094-0.917-9.093-0.932c-2.999,0.005-5.995,0.291-9.097,0.921C309.467,331.275,306.319,332.275,302.604,333.539z"/>
						...
						<p class="text-lg md:text-xl text-center font-black text-dark dark:text-white mt-8 md:mt-12">متاسفانه دوره ای مطابق با جستجوی شما پیدا نشد ):</p>
					</div>
        @endforelse
    {{ $courses->links('customer.layouts.paginate') }}
