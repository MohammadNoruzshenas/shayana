...
@section('head-tag')
<title>ثبت نام - {{cache('templateSetting')['title']}}</title>
...
<section class="content lg:blur-0 py-10">
    ...
    <section class="container flex justify-between items-center lg:flex-row flex-col relative min-h-96 ">
        <form action="{{ route('auth.customer.register') }}" method="post" class="lg:w-5/12 w-full" data-aos="fade-up">
        @csrf
            ...
            <main class="flex items-center justify-center  w-full">
                ...
                <div class="flex flex-col w-full h-auto gap-10   lg:rounded-3xl bg-white  shadow-lg rounded-2xl dark:bg-dark dark:shadow-none p-5">
                    ...
                    <h2 class="text-4xl font-bold text-main">ثبت نام</h2>
                    @error('WrongMobile')
                    <div>
                  ...
                  @enderror" placeholder=" ایمیل" type="text" />
                        ...
                        @enderror">
                                    ...
                                    <input name="mobile" value="{{old('mobile')}}" class="w-full py-4 pr-4 border placeholder:text-main  focus:border-main  pl-14 rounded-xl focus:ring-0  dark:bg-secondary dark:text-white/80 @error('mobile')
                      focus:border-red-500   border-red-500 placeholder:text-red-500 placeholder:font-bold
                  ...
                  @enderror" placeholder="تکرار رمز عبور " type="password" />
                        ...
                        <span class="font-bold text-gray-400">حساب کاربر دارید ؟ <a
                                href="{{ route('auth.customer.loginForm') }}" class="text-main">وارد شوید</a></span>
                        
                        <div class="flex flex-col gap-3">
                            <label class="flex items-start gap-3 cursor-pointer">
                                <input type="checkbox" name="rules" value="1" 
                                    class="mt-1 w-5 h-5 text-main bg-gray-100 border-gray-300 rounded focus:ring-main focus:ring-2 dark:bg-secondary dark:border-gray-600 @error('rules') border-red-500 @enderror" 
                                    {{ old('rules') ? 'checked' : '' }}>
                                <span class="text-gray-700 dark:text-white/80">
                                    <a href="{{ route('customer.rules') }}" target="_blank" class="text-main hover:underline font-bold">قوانین و مقررات سایت</a> را خوانده و می‌پذیرم
                                </span>
                            </label>
                            @error('rules')
                            <span class="font-bold text-red-500">{{ $message }}</span>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const confirmPasswordInput = document.querySelector('.confirmPasswordInp');

    // Show confirm password when show icon is clicked
    showConfirmPassIcon.addEventListener('click', function() {
      confirmPasswordInput.type = 'text';
      showConfirmPassIcon.classList.add('hidden');
      hideConfirmPassIcon.classList.remove('hidden');
    });

    // Hide confirm password when hide icon is clicked
    hideConfirmPassIcon.addEventListener('click', function() {
      confirmPasswordInput.type = 'password';
      hideConfirmPassIcon.classList.add('hidden');
      showConfirmPassIcon.classList.remove('hidden');
    });
  });
</script>
@endsection
