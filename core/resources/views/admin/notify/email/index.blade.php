...
<div class="tab__box">
    <div class="tab__items">
        <a class="tab__item is-active" href="{{ route('admin.notify.email.create') }}">ایجاد اطلاعیه جدید</a>
    </div>
</div>
        ...
        <div class="table__box">
                            ...
                            <td>
                                ...
                                <label>
                                    <input id="{{ $email->id }}"
                                        ...
                                        data-url="{{ route('admin.notify.email.status', $email) }}"
                                        type="checkbox" @if ($email->status === 1) checked @endif>
                                </label>
                            </td>
                                        <td>
                                            <form action="{{ route('admin.notify.email.destroy', $email) }}" method="post">
                                                @csrf
                                                @method('delete')
                                <a href="{{ route('admin.notify.email.edit', $email) }}"
                                class="btn btn-warning">ویرایش</a>

                                <a href="{{ route('admin.notify.email-file.index', $email) }}"
                                class="btn btn-warning">پیوست فایل</a>
          ...
          <!-- paginate -->
  {{ $emails->links('admin.layouts.paginate') }}
  <!-- endpaginate -->
    </div>
@endsection
@section('script')
