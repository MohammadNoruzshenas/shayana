...
@section('breadcrumb')
    ...
    <li><a href="{{ route('admin.content.podcast.create') }}">ایجاد پادکست </a></li>
@endsection
@section('content')
    <p class="box__title">ایجاد پادکست </p>
    <div class="row no-gutters bg-white">
        <div class="col-12">
            <form action="{{ route('admin.content.podcast.store') }}" method="post" enctype="multipart/form-data"
                class="padding-30">
                @csrf
                <p class="mb-5 font-size-14"> عنوان پادکست : </p>
                <input type="text" value="{{ old('title') }}" name="title" class="text" placeholder="عنوان پادکست">
                @error('title')
                ...
                <p class="mb-5 font-size-14">انتخاب گوینده : </p>
                    <select name="podcaster_id">
                        <option value="">انتخاب گوینده</option>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @if (old('podcaster_id') == $user->id) selected @endif>
                ...
                <input type="text" dir="ltr" value="{{ old('voice') }}" name="voice" class="text text-left" placeholder="لینک پادکست">
                @error('voice')
                    <span class="text-error" role="alert">
                        <strong>
                            {{ $message }}
                ...
                <textarea name="summary" id="summary" placeholder="خلاصه پادکست + نمایش در متا" class="text h">{{ old('summary') }}</textarea>
                @error('summary')
                    <span class="text-error" role="alert">
                        <strong>
                            {{ $message }}
                ...
                <button class="btn btn-brand mt-5">ایجاد پادکست</button>
            </form>
        </div>
    </div>
