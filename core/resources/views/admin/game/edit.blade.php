...
@section('content')
    <div class="p-4 w-full font-sans">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {{-- بخش فرم ویرایش بازی --}}
            <div class="lg:col-span-6 lg:col-start-4" x-data="gameForm">
                        ...
                        <form action="{{ route('admin.game.update', $game->id) }}" method="post"
                            ...
                            <div class="mb-6">
                                ...
                                <div class="relative">
                                    ...
                                    <button type="button" onclick="document.getElementById('gameImage').click()"
                                        ...
                                        <span class="text-sm text-gray-600">برای تغییر تصویر کلیک کنید</span>
                                        <span class="text-xs text-gray-400">یا تصویر را بکشید و رها کنید</span>
                                    </button>
                                </div>
                                <!-- پیش‌نمایش تصویر -->
                                <div id="imagePreviewContainer" class="{{ $game->image ? '' : 'hidden' }} mt-4">
                                    <div class="relative inline-block w-full">
                                        <img id="imagePreview"
                                            src="{{ $game->image ? asset('storage/' . $game->image) : '' }}"
                                            alt="پیش‌نمایش"
                            ...
                            <div class="mb-6">
                                ...
                                <div class="rounded-xl overflow-hidden border border-gray-300">
                                    <textarea name="description" id="description" class="ckeditor">{{ old('description', $game->description) }}</textarea>
                                </div>
                            </div>
...
@section('script')
    ...
    <script>
        ...
        function removeImage() {
            ...
            // اگر تصویر از قبل بوده، نباید مخفی شود مگر اینکه کاربر بخواهد کلا حذف کند که اینجا استراتژی ساده‌ای داریم
            // در حالت ویرایش اگر تصویر قبلی بوده، با زدن ضربدر فقط فایل جدید را پاک میکنیم و برمیگردیم به حالت قبل یا خالی
            // فعلا فقط value اینپوت رو خالی میکنیم و عکس رو خالی نشون میدیم
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('gameForm', () => ({
                ...
                selectedCourseId: '{{ $game->course_id }}',
                selectedCourseText: '{{ $game->course->title ?? 'انتخاب دوره...' }}',
                selectedMainSeasonId: '{{ $game->main_season_id }}',
                selectedMainSeasonText: '{{ $game->mainSeason->title ?? 'انتخاب فصل اصلی...' }}',
                selectedSubSeasonId: '{{ $game->sub_season_id }}',
                selectedSubSeasonText: '{{ $game->subSeason->title ?? 'انتخاب زیرفصل...' }}',

                init() {
                    this.fetchCourses();
                    // اگر مقادیر اولیه وجود دارد، فصل‌ها و زیرفصل‌ها را بارگیری کنیم
                    if (this.selectedCourseId) {
                        this.fetchMainSeasons(this.selectedCourseId);
                    }
                    if (this.selectedMainSeasonId) {
                        this.fetchSubSeasons(this.selectedMainSeasonId);
                    }
                },
                ...
                async selectCourse(course) {
                    ...
                    this.fetchMainSeasons(course.id);
                },

                async fetchMainSeasons(courseId) {
                    const response = await fetch(
                        `{{ route('admin.game.get-seasons-by-course') }}?course_id=${courseId}`, {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                async selectMainSeason(season) {
                    ...
                    this.fetchSubSeasons(season.id);
                },

                async fetchSubSeasons(mainSeasonId) {
                    const response = await fetch(
                        `{{ route('admin.game.get-sub-seasons-by-main-season') }}?main_season_id=${mainSeasonId}`,
                        {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json',
