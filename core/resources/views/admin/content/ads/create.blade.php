...
<li><a href="{{ route('admin.content.ads.index') }}">ایجاد تبلیغ</a></li>
...
<div class="row no-gutters bg-white">
    <div class="col-12">
        <form action="{{ route('admin.content.ads.store') }}" enctype="multipart/form-data" method="post" class="padding-30">
            @csrf
            <input type="text" name="title" value="{{ old('title') }}" class="text" placeholder="عنوان تبلیغ">
            @error('title')
        ...
        @enderror
            <input type="text" name="link" value="{{ old('link') }}" class="text text-left " placeholder="لینک تبلیغ">
            @error('link')
            <span class="text-error" role="alert">
                <strong>
                    {{ $message }}
        ...
        @enderror
           <input type="text" name="amount" value="{{ old('amount') }}" class="text text-left " placeholder="مبلغ دریافت شده">
            @error('amount')
            <span class="text-error" role="alert">
                <strong>
                    {{ $message }}
        ...
        @enderror
        <input type="text" name="description" value="{{ old('description') }}" class="text text-left " placeholder="توضیحات پرداخت">
        @error('description')
        <span class="text-error" role="alert">
            <strong>
                {{ $message }}
    ...
    @enderror
    <p class="mb-5 font-size-14"> محل قرارگیری بنر  : </p>

    <select name="position">
        <option value="0" @if (old('position') == 0) selected @endif>صفحات دوره ها و مقالات</option>
        <option value="1" @if (old('position') == 1) selected @endif>اولین بنر صفحه اصلی  </option>
        <option value="2" @if (old('position') == 2) selected @endif>دومین بنر صفحه اصلی  </option>
