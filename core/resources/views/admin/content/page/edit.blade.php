...
@section('breadcrumb')
    ...
    <li><a href="{{ route('admin.content.page.create') }}"> ویرایش صفحه </a></li>

@endsection
@section('content')
<p class="box__title">ویرایش صفحه {{ $page->title}}</p>
<div class="row no-gutters bg-white">
    <div class="col-12">
        <form action="{{ route('admin.content.page.update',$page) }}" method="post" enctype="multipart/form-data" class="padding-30">
            @csrf
            @method('put')
            <input type="text" value="{{ old('title',$page->title) }}"  name="title" class="text" placeholder="عنوان صفحه">
            @error('title')
        ...
        @enderror
            <div style="margin:5px"></div>
            <button class="btn btn-brand mt-5">ویرایش صفحه</button>
        </form>
    </div>
</div>
