...
@section('content')
        <div class="row no-gutters  ">
            <div class="col-8 margin-left-10 margin-bottom-15 border-radius-3">
                <p class="box__title">دسته بندی مقالات ها</p>
                        ...
                        <tbody>
                            ...
                            @foreach ($postCategories as $postCategory)
                                <tr role="row" class="">
                                    ...
                                    <td><a href="">{{ $postCategory->title }}</a></td>
                                    <td> {{ $postCategory->parent->title ?? '-' }}</td>
                                    {{-- <td>
                                        <label>
                                    ...
                                    <td>
                                        @permission('delete_categoryPost')
                                            <form action="{{ route('admin.content.category.destory', $postCategory->id) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                                <button class="item-delete mlg-15 delete" title="حذف"><li class="fa-solid fa-trash"></li></button>
                                            @endpermission
                                            <a href="{{ route('customer.blogs',$postCategory->slug) }}" target="_blank" class="item-eye mlg-15" title="مشاهده"><li class="fa-solid fa-eye"></li></a>
        ...
        @permission('create_categoryPost')
            @include('admin.content.category.create')
        @endpermission
    </div>
    </div>
        <!-- paginate -->
        {{ $postCategories->links('admin.layouts.paginate') }}
        <!-- endpaginate -->
    </div>
@endsection
@section('script')
