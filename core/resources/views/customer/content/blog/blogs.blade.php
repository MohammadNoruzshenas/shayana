...
@section('head-tag')
<title>{{ request()->postCategory->title ?? 'مقالات'}} -  {{cache('templateSetting')['title']}}</title>
<meta property="og:title"
    content="{{ cache('templateSetting')['title'] }} - {{ request()->postCategory->title ?? 'پادکست ها' }}">
<meta property="og:description" content="{{ request()->postCategory->meta_description ?? '' }}">
<meta property="og:image" content="{{ asset(cache('templateSetting')['logo']) }}">
<meta property="og:url"
    content="{{ route('customer.podcasts', ['postCategory' => request()->postCategory ? request()->postCategory->slug : null]) }}">
<meta name="twitter:title"
    content="{{ cache('templateSetting')['title'] }} - {{ request()->postCategory->title ?? 'پادکست ها' }}">
<meta name="twitter:description" content="{{ request()->postCategory->meta_description ?? '' }}">
...
@section('content')
    <section class="container content lg:blur-0 my-7">
        <div class="flex items-center justify-between w-full my-7">
            <h1 class="text-lg font-bold lg:text-3xl text-secondary dark:text-white/80">همه پست ها</h1>
            <a href="{{ route('customer.home') }}" class="flex items-center gap-2 text-lg font-bold lg:text-lg group text-main"> بازگشت
        ...
        <form action="{{ route('customer.blogs',['postCategory' => request()->postCategory ? request()->postCategory->slug : null]) }}" method="get">
        <div class="flex flex-col justify-between h-auto gap-6 py-3 lg:flex-row">
                        ...
                        <div id="accordion-flush-body-1" class="hidden" aria-labelledby="accordion-flush-heading-1">
                            <div class="flex flex-col px-3 gap-y-3">
                                <div class="flex items-center gap-x-2 text-secondary dark:text-white/80 hover:text-main hover:font-bold  hover:text-main hover:font-bold dark:hover:text-main"><input
                                    type="radio" name="vip" @if(request('vip')==0) checked @endif id="allSort"
                                ...
                                <div class="flex items-center gap-x-2 text-secondary dark:text-white/80 hover:text-main hover:font-bold  hover:text-main hover:font-bold dark:hover:text-main"><input
                                        type="radio" name="vip" @if(request('vip')==1) checked @endif id="freeSort"
                                        class="w-4 h-4 border-none rounded-full cursor-pointer form-radio bg-secondary/50 checked:text-main"
                                        value="1">
                                ...
                                <div class="flex items-center gap-x-2 text-secondary dark:text-white/80 hover:text-main hover:font-bold  hover:text-main hover:font-bold dark:hover:text-main"><input
                                        type="radio" name="vip" @if(request('vip')==2) checked @endif id="specialSort"
                                        class="w-4 h-4 border-none rounded-full cursor-pointer form-radio bg-secondary/50 checked:text-main"
                                        value="2">
                            ...
                            <div class="flex flex-col px-3 gap-y-3">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-x-2 text-secondary dark:text-white/80 hover:text-main hover:font-bold  hover:text-main hover:font-bold dark:hover:text-main"><a
                                            href="{{ route('customer.blogs') }}">همه</a></div>
                                </div>
                                @foreach ($categories as $category)
                                    <div class="flex items-center gap-x-2 text-secondary dark:text-white/80 hover:text-main hover:font-bold  hover:text-main hover:font-bold dark:hover:text-main"><a
                                            href="{{ route('customer.blogs',$category->slug)}}">{{ $category->title }}</a>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        <div class="flex item-center gap-2 flex-col mt-3">
                            <button type="submit" class="px-6 py-2  font-medium text-white transition border rounded-lg bg-main border-main shrink-0 hover:bg-transparent hover:text-main focus:outline-none  active:text-main">اعمال فیلتر</button>
                            <a href="{{route('customer.blogs')}}" class="px-6 py-2 text-center block  font-medium text-main transition border rounded-lg bg-transparent border-main shrink-0 hover:bg-main hover:text-white focus:outline-none  active:text-main">حذف فیلترها</a>
                            </div>
                    </div>
                </form>
                </div>
            ...
            <div class="flex flex-wrap w-full mt-10 gap-y-14 lg:gap-x-4 lg:w-4/5 h-min md:justify-between lg:justify-start" data-aos="fade-down">

                @forelse ($posts as $post)

                    <div
                    class="relative flex flex-col items-center justify-center w-full shadow-lg rounded-3xl dark:bg-dark   dark:shoadow-none lg:w-[32%] md:w-[49%] ">
                    <div class="relative flex items-center justify-center w-11/12 h-52 -top-10 ">
                        <img src="{{ asset($post->image) }}" class="absolute z-50 object-cover w-full h-full rounded-3xl"
                            alt="">
                            @if ($post->is_vip == 1)
                            <div
                    ...
                    <div class="flex flex-col justify-start w-full gap-3 px-10 -mt-5">

                        <a href="{{route('customer.singlePost',$post->slug)}}">
                            <h3 class="pb-2 text-2xl font-bold border-b text-secondary dark:text-white/80 border-b-main/10 hover:text-main hover:font-bold dark:hover:text-main">
                             {{$post->title}}</h3>
                        </a>

                        <div class="flex gap-2.5 flex-wrap text-slate-500 dark:text-slate-400 text-xs">
                            <div class="flex items-center transition-colors cursor-pointer gap-x-1 hover:text-primary">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            ...
                            <div class="flex flex-wrap items-center justify-between w-full mt-10 mb-5">
                                ...
                                <a href="{{ route('customer.singlePost', $post->slug) }}"
                                    class="flex items-center gap-1 px-4 py-2 text-sm font-bold text-white bg-main rounded-xl hover:bg-opacity-75 group">

                                    مطالعه مقاله
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor"
                                        class="w-6 h-6 ltr:rotate-180 rtl:rotate-0 group-hover:-translate-x-2 duration-200">
                                        <path stroke-linecap="round" stroke-linejoin="round"
...
<g>
    ...
    <path style="fill:#FFFFFF;" d="M302.604,333.539c1.292-3.725,4.122-6.738,7.526-8.83c3.426-2.077,7.494-3.192,11.563-3.191    c4.069-0.01,8.14,1.102,11.567,3.181c3.406,2.09,6.236,5.108,7.521,8.84c-3.722-1.257-6.869-2.252-9.996-2.86    c-3.101-0.629-6.094-0.917-9.093-0.932c-2.999,0.005-5.995,0.291-9.097,0.921C309.467,331.275,306.319,332.275,302.604,333.539z"/>
                    ...
                    <p class="text-lg md:text-xl text-center font-black text-dark dark:text-white mt-8 md:mt-12">متاسفانه مقاله ای مطابق با جستجوی شما پیدا نشد ):</p>
                </div>
                @endforelse
            </div>
    ...
    {{ $posts->links('customer.layouts.paginate') }}
