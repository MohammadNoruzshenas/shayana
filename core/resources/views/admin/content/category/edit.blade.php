    ...
    <div class="main-content font-size-13">
        <div class="row no-gutters bg-white margin-bottom-20">
            <div class="col-12">
                ...
                <form action="{{ route('admin.content.category.update', $postCategory->id) }}" class="padding-30"
                    method="post">
                    {{ method_field('put') }}
                    @csrf

                    <input type="text" name="title" required value="{{ old('name', $postCategory->title) }}" class="text"
                    ...
                    <select name="show_in_menu" id="">
                        <option value="1" @if (old('show_in_menu', $postCategory->status) == 1) selected @endif>فعال</option>
                        <option value="0" @if (old('show_in_menu', $postCategory->status) == 0) selected @endif>غیرفعال</option>
                    </select> --}}
                    <p class="mb-5 font-size-14"> توضیحات متا : </p>
                    <textarea name="meta_description" id="meta_description" placeholder="meta description"  class="text h ltr-text">{{ old('meta_description', $postCategory->meta_description) }}</textarea>
                    ...
                    <select name="parent_id" id="">
                        ...
                        <option value="{{ $categoryItem->id }}" @if ($categoryItem->id == $postCategory->parent_id) selected @endif>
